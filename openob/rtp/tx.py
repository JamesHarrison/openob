...
class RTPTransmitter:
  def __init__(self, audio_input='alsa', audio_device='hw:0', base_port=3000, encoding='opus', bitrate=96, jack_name='openob_tx', jack_auto=True, receiver_address='localhost', opus_options={'audio': True, 'bandwidth': -1000, 'frame-size': 20, 'complexity': 7, 'constrained-vbr': True, 'inband-fec': True, 'packet-loss-percentage': 3, 'dtx': False}):
    ...
    if audio_input == 'jack':
      ...
      gst.element_link_many(self.source, self.capsfilter, self.level,self.audioresample, self.audiorate, self.audioconvert, self.queue)
    else:
      gst.element_link_many(self.source, self.level, self.queue,  self.audioresample, self.audiorate, self.audioconvert, self.queue)
    # Now we get to link this up to our encoder/payloader

    if encoding != 'pcm':
      gst.element_link_many(self.queue, self.encoder, self.payloader)
    else:
      gst.element_link_many(self.queue, self.payloader)
